...
function CMConfigPanel({ onConfigSaved }) {
  ...
  const addVendor = async () => {
    ...
    try {
      // Create vendor via a dedicated endpoint (we POST directly to the DB via a simple route)
      // For now we use a thin POST to /config/cm-vendors/create
      await api.post('/config/cm-vendors', {
        name:           newName.trim(),
        zoho_vendor_id: newZohoId.trim() || null,
      });
  ...
  return (
    <div className="card p-5 space-y-4">
        ...
        <div className="rounded-lg border border-dashed border-brand-300 dark:border-brand-700 bg-brand-50/40 dark:bg-brand-900/10 p-4 space-y-3">
          ...
          <div className="flex gap-2">
            <button onClick={addVendor} disabled={addLoading} className="btn-primary text-xs px-3 py-1.5">
              {addLoading ? <RefreshCw className="h-3.5 w-3.5 animate-spin" /> : <Save className="h-3.5 w-3.5" />}
              Save Vendor
            </button>
      ...
      {error   && <p className="text-xs text-red-600 dark:text-red-400 flex items-center gap-1"><AlertCircle className="h-3.5 w-3.5" />{error}</p>}
      {success && <p className="text-xs text-emerald-600 dark:text-emerald-400 flex items-center gap-1"><CheckCircle2 className="h-3.5 w-3.5" />{success}</p>}
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          <p className="text-xs text-gray-500 dark:text-gray-400">
            Check vendors to mark as <strong>Contract Manufacturers</strong>. Multiple allowed.
          </p>
            ...
            {vendors.map(v => (
              <label key={v.id} className="flex items-center gap-3 px-4 py-3 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700/40 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {v.zoho_vendor_id && (
                    <p className="text-xs text-gray-400 dark:text-gray-500">Zoho ID: {v.zoho_vendor_id}</p>
                  )}
                </div>
    ...
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <div className="card py-14 text-center">
          ...
          <p className="text-sm text-gray-400 dark:text-gray-500">No inventory data found.</p>
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Configure CM vendors and add PO line items to see inventory.</p>
        </div>
      ) : (
...
function VendorSummaryCard({ vendorName, items }) {
  ...
  const totals = items.reduce(
    (acc, r) => ({
      physical: acc.physical + Number(r.physical_inventory),
      planned:  acc.planned  + Number(r.planned_inventory),
    }),
...
function DetailView() {
  ...
  const [editing,    setEditing]    = useState(null); // po_line_item_id being edited
  const [editVals,   setEditVals]   = useState({ planned_qty: '', actual_qty: '' });
  const [saving,     setSaving]     = useState(false);
  const [saveErr,    setSaveErr]    = useState('');

  // Load CM vendors for the selector
  useEffect(() => {
  ...
  const startEdit = (row) => {
    setEditing(row.po_line_item_id);
    setEditVals({ planned_qty: row.planned_qty, actual_qty: row.actual_qty });
    setSaveErr('');
  };
    ...
    <div className="space-y-4">
        ...
        <div className="flex-1 max-w-xs">
          ...
          {vendors.length === 0 ? (
            <p className="text-xs text-amber-600 dark:text-amber-400 flex items-center gap-1">
              <AlertCircle className="h-3.5 w-3.5" /> No CM vendors configured. Use the Config tab.
            </p>
          ) : (
            <select
              className="input text-sm"
      ...
      {loading ? (
        ...
        <div className="card py-14 text-center">
          ...
          <p className="text-sm text-gray-400 dark:text-gray-500">No PO line items found for this vendor.</p>
        </div>
      ) : (
        <>
              ...
              <table className="w-full text-xs">
                <thead className="bg-gray-50 dark:bg-gray-700/50 border-b border-gray-100 dark:border-gray-700">
                  <tr>
                    ...
                    <th className="table-th text-right">Planned Qty</th>
                ...
                <tbody className="divide-y divide-gray-50 dark:divide-gray-700">
                  {rows.map(row => (
                    <tr key={row.po_line_item_id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                      <td className="table-td font-medium text-brand-600 dark:text-brand-400">{row.po_number}</td>
                      <td className="table-td text-gray-500 dark:text-gray-400">{row.po_item_id || '—'}</td>
                      <td className="table-td max-w-[200px] truncate" title={row.description}>{row.description || '—'}</td>
                      <td className="table-td text-right"><QtyBadge value={row.po_qty} /></td>
                      <td className="table-td text-right">
                        {editing === row.po_line_item_id ? (
                          <input
                            type="number" min="0"
                            className="input w-20 text-right text-xs py-1"
                            value={editVals.planned_qty}
                            onChange={e => setEditVals(v => ({ ...v, planned_qty: e.target.value }))}
                          />
                        ) : <QtyBadge value={row.planned_qty} />}
                      </td>
                      <td className="table-td text-right">
                        {editing === row.po_line_item_id ? (
                          <input
                            ...
                            onChange={e => setEditVals(v => ({ ...v, actual_qty: e.target.value }))}
                          />
                        ) : <QtyBadge value={row.actual_qty} />}
                      </td>
                      ...
                      <td className="table-td">
                        {editing === row.po_line_item_id ? (
                          <div className="flex items-center gap-1.5">
                            <button
                              onClick={() => saveEdit(row.po_line_item_id)}
                              disabled={saving}
                              className="flex items-center gap-1 px-2 py-1 rounded-md bg-brand-600 text-white text-[11px] font-semibold hover:bg-brand-700 transition-colors disabled:opacity-50"
                            >
                              {saving ? <RefreshCw className="h-3 w-3 animate-spin" /> : <Save className="h-3 w-3" />} Save
                            </button>
                            <button onClick={cancelEdit} className="flex items-center gap-1 px-2 py-1 rounded-md text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-700 text-[11px]">
                          ...
                          <button
                            ...
                            className="flex items-center gap-1 px-2 py-1 rounded-md text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-700 text-[11px] font-medium transition-colors"
                          >
          ...
          <ul className="space-y-3 sm:hidden">
            {rows.map(row => (
              <li key={row.po_line_item_id} className="card p-4 space-y-3">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <p className="text-sm font-bold text-brand-600 dark:text-brand-400">{row.po_number}</p>
                    {row.po_item_id && <p className="text-xs text-gray-400">{row.po_item_id}</p>}
                  </div>
                  <QtyBadge value={row.po_qty} />
                </div>
                {row.description && <p className="text-xs text-gray-600 dark:text-gray-400 truncate">{row.description}</p>}
                <div className="grid grid-cols-3 gap-2 text-xs text-center">
                  {[['Planned', row.planned_qty], ['Actual', row.actual_qty], ['Billed', row.billed_qty]].map(([label, val]) => (
                    <div key={label} className="rounded-lg bg-gray-50 dark:bg-gray-700/40 px-2 py-2">
                      <p className="text-gray-400 dark:text-gray-500 text-[10px] mb-1">{label}</p>
                      <p className="font-bold text-gray-800 dark:text-gray-200">{fmt(val)}</p>
                    </div>
                  ))}
                </div>
                <button onClick={() => startEdit(row)} className="btn-outline w-full text-xs justify-center py-1.5">
                  <Pencil className="h-3.5 w-3.5" /> Update Quantities
                </button>
              </li>
            ))}
          </ul>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="hero-subtitle">
              Aggregate and per-line-item inventory view for contract manufacturers —
              based on production actuals and billed quantities.
            </p>
            <div className="mt-3 flex items-center gap-2 flex-wrap">
              <span className="chip-soft">Physical Inventory</span>
              <span className="chip-soft">Planned Inventory</span>
              <span className="chip-soft">Billed Qty Deduction</span>
