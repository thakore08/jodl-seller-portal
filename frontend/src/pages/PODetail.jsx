...
export default function PODetail() {
  ...
  // Fetch seller WhatsApp info for admin modal (runs once po + vendorId are known)
  useEffect(() => {
    if (!isJodlAdmin || !po?.vendor_id) return;
    api.get('/whatsapp/sellers')
      .then(({ data }) => {
        const match = (data.sellers || []).find(s => s.vendor_id === po.vendor_id);
        setWaSellerInfo(match || null);
      })
      .catch(() => {});
  }, [isJodlAdmin, po?.vendor_id]);
  ...
  return (
    <div className="space-y-5 max-w-4xl mx-auto">
      ...
      {/* WhatsApp Message Modal (admin only) */}
      {showWaModal && (
        <WhatsAppMessageModal
          po={po}
          sellerInfo={waSellerInfo}
